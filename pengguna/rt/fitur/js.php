...
<script>
    ...
    function copyToClipboard(text, element) {
        navigator.clipboard.writeText(text).then(() => {
            // Ubah title sementara
            const tooltipInstance = bootstrap.Tooltip.getInstance(
                element);
            tooltipInstance.setContent({
                '.tooltip-inner': 'Tersalin!'
            });
            tooltipInstance.show();

            // Kembalikan title setelah 2 detik
            setTimeout(() => {
                tooltipInstance.setContent({
                    '.tooltip-inner': 'Klik untuk menyalin'
                });
        ...
        document.getElementById('tambahForm').addEventListener('submit', function(event) {
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    ...
                    if (response === 'success') {
                    ...
                    } else if (response === 'data_bantuan_sudah_ada') {
                        Swal.fire({
                            title: "Error",
                            text: "Data Bantuan sudah ada, silakan pilih data lainnya",
                            icon: "info",
                            timer: 4000, // 2 detik
                            showConfirmButton: false,
                        });
                    } else if (response === 'nikk_sudah_ada') {
                        Swal.fire({
                            title: "Error",
                            text: "Data Nomor Induk Kartu Keluarga sudah ada, silakan pilih data lainnya",
                            icon: "info",
                            timer: 4000, // 2 detik
                            showConfirmButton: false,
                        });
                    } else if (response === 'nikk_wajib_16_digit') {
                        Swal.fire({
                            title: "Error",
                            text: "Data Nomor Induk Kartu Keluarga wajib 16 digit, silakan masukan data yang tepat",
                            icon: "info",
                            timer: 4000, // 2 detik
                            showConfirmButton: false,
                        });
                    } else if (response === 'nkk_sudah_ada') {
                        Swal.fire({
                            title: "Error",
                            text: "Data Nomor Induk Kependudukan sudah ada, silakan pilih data lainnya",
                            icon: "info",
                            timer: 4000, // 2 detik
                            showConfirmButton: false,
                        });
                    } else if (response === 'nkk_wajib_16_digit') {
                        Swal.fire({
                            title: "Error",
                            text: "Data Nomor Induk Kependudukan wajib 16 digit, silakan masukan data yang tepat",
                            icon: "info",
                            timer: 4000, // 2 detik
                            showConfirmButton: false,
                        });
                        ...
                        Swal.fire({
                            title: "Error",
                            text: "Data sudah ada, silakan pilih data lainnya",
                            icon: "info",
                            timer: 4000, // 2 detik
                            showConfirmButton: false,
                        });
                    } else if (response === 'data_nama_rt_sudah_ada') {
                        Swal.fire({
                            title: "Error",
                            text: "Nama RT Sudah Ada, silakan pilih nama RT lainnya",
                            icon: "info",
                            timer: 4000, // 2 detik
                            showConfirmButton: false,
                        });
