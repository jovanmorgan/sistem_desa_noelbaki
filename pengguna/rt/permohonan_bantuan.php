...
";
        ...
        ?>">
                                            ...
                                            <td class="align-middle text-center">
                                                ...
                                                <button class="btn btn-primary btn-sm mt-3"
                                                    onclick="openEditModal('<?php echo $row['id_permohonan_bantuan']; ?>', '<?php echo $row['id_kkm']; ?>', '<?php echo $row['jenis_bantuan']; ?>')">Edit</button>
                                                <button class="btn btn-danger btn-sm mt-3"
                                                    onclick="hapusData(<?php echo $row['id_permohonan_bantuan']; ?>)">Hapus</button>
                                            </td>
                                        </tr>
                                        <?php endwhile; ?>
                                    </tbody>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="tambahForm" method="POST" action="proses/<?= $page_title_proses ?>/tambah.php"
                            ...
                            <!-- Select Keluarga Kurang Mampu -->
                            <div class="mb-3">
                                <label for="id_kkm" class="form-label">
                                    Keluarga Kurang Mampu
                                </label>

                                <div class="input-group">
                                    <span class="input-group-text"><i class="fas fa-users"></i></span>
                                    <select class="form-select" id="id_kkm" name="id_kkm" required>
                                        <option value="">Pilih Keluarga Kurang Mampu</option>
                                        <?php
                                        // Query untuk mendapatkan data dari tabel keluarga_kurang_mampu dan kepala_keluarga
                                        $query = "
                                                    SELECT 
                                                        kkm.id_kkm, 
                                                        kk.nama_lengkap, 
                                                    ...
                                                    WHERE kkm.id_rt = '$id_rt'
                                                    AND kkm.status_validasi = 'Disetujui'  -- Filter status validasi
                                                ";
                                        $result = $koneksi->query($query);

                                        // Cek apakah ada data yang ditemukan
                                        if ($result->num_rows > 0) {
                                            // Iterasi hasil query untuk ditampilkan pada dropdown
                                            while ($row = $result->fetch_assoc()):
                                        ?>
                                        <option value="<?php echo htmlspecialchars($row['id_kkm']); ?>">
                                            <?php echo htmlspecialchars($row['nama_lengkap']); ?> (NIKK:
                                            <?php echo htmlspecialchars($row['nomor_induk_kartu_keluarga']); ?>,
                                            Pemasukan/Bulan:
                                            <?php echo htmlspecialchars($row['pemasukan_perbulan']); ?>)
                                        </option>
                                        <?php
                                            endwhile;
                                        } else {
                                            ...
                                            ?>
                                        <option value="">~Keluarga Kurang Mampu kosong atau belum disetujui~</option>
                                        <?php
                                        }
                                        ?>
                                    </select>
                            ...
                            <!-- Select Jenis Bantuan -->
                            <div class="mb-3">
                                <label for="jenis_bantuan" class="form-label">Pilih Jenis Bantuan</label>
                                <div class="input-group">
                                    ...
                                    <select class="form-select" id="jenis_bantuan" name="jenis_bantuan" required>
                                        ...
                                        <option value="Bantuan Perbaikan Rumah">Bantuan Perbaikan Rumah</option>
                                        <option value="Bantuan Modal Usaha">Bantuan Modal Usaha</option>
                                        <option value="Bantuan Lansia">Bantuan Lansia</option>
                                        <option value="Bantuan Disabilitas">Bantuan Disabilitas</option>
                                        <option value="Bantuan Dana Darurat">Bantuan Dana Darurat</option>
                                        <option value="Bantuan Bencana Alam">Bantuan Bencana Alam</option>
                                        <option value="Bantuan Transportasi">Bantuan Transportasi</option>
                                    </select>
                                </div>
                            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="editForm" method="POST" action="proses/<?= $page_title_proses ?>/edit.php"
                            ...
                            <input type="hidden" name="id_rt" id="id_rt" value="<?php echo $id_rt; ?>">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text"><i class="fas fa-users"></i></span>
                                    <select class="form-select" id="edit_id_kkm" name="id_kkm" required>
                                        <option value="">Pilih Keluarga Kurang Mampu</option>
                                        <?php
                                        // Query untuk mendapatkan data dari tabel keluarga_kurang_mampu dan kepala_keluarga
                                        $query = "
                            ...
                            <!-- Select Jenis Bantuan -->
                            <div class="mb-3">
                                <label for="edit_jenis_bantuan" class="form-label">Pilih Jenis Bantuan</label>
                                <div class="input-group">
                                    <span class="input-group-text"><i class="fas fa-hands-helping"></i></span>
                                    <select class="form-select" id="edit_jenis_bantuan" name="jenis_bantuan" required>
                                        <option value="">Pilih Jenis Bantuan</option>
                                        <option value="Bantuan Sembako">Bantuan Sembako</option>
                                        <option value="Bantuan Pendidikan">Bantuan Pendidikan</option>
                                        <option value="Bantuan Kesehatan">Bantuan Kesehatan</option>
    ...
    <script>
    function openEditModal(id, id_kkm, jenis_bantuan) {
        // Inisialisasi Modal
        let editModal = new bootstrap.Modal(document.getElementById('editModal'), {});

        // Mengisi nilai input dengan data yang diterima
        document.getElementById('edit_id').value = id;
        document.getElementById('edit_id_kkm').value = id_kkm;
        document.getElementById('edit_jenis_bantuan').value = jenis_bantuan;
